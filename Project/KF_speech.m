function [] = KF speech(filename, noiseType, orderType)
...
saveToPath = ['Results\Rnew all noise ',orderType, ' order\',noiseType,...
'\',filename,'\'];
...
[fileID] = fopen([saveToPath,filename,' ',noiseType,' results.txt'],'w+');
fprintf(fileID,'%s %s %s %s %s %s %s %s\r\n','R(new)','Q chosen',...
'log(Q)','SNR','SegSNR before','SegSNR after','PESQ','Average order');
...
wn = wavread([soundpath,'white noise.wav']);
[noise,snr] = makeSNR(z, wn, SNR(snri));
y = noise + z;
else
[y,fs] = wavread(strcat(soundpath, noiseType,'\',...
num2str(SNR(snri)),'dB\',filename,' ',type,' sn',....
num2str(SNR(snri)),'.wav'));
end
y=y';
z=z';
...
%interpolate nq, J1 and J2 to increase resolution, and
%to get more accurate approximation of Q
nqi = −5:0.25:4;
J2i = interp1(nq,J2,nqi);
J1i = interp1(nq,J1,nqi);
[nq nom,Jc]=intersections(nqi,J1i,nqi,J2i);

if m < 3
J2 desired = (0.25*(m+1)*(Jc−min(J2i))) + min(J2i);
else
J2 desired = (0.25*(m−3)*(max(J2i)−Jc))+ Jc;
end

[difference, index] = min(abs(J2i − J2 desired));
Q = 10ˆ(nqi(index));
...
segsnr before=segsnr before+log10(rms(zseg(i,:))/...
rms(zseg(i,:)−segment(i,:)));
segsnr after=segsnr after+log10(rms(zseg(i,:))/...
rms(zseg(i,:)−cleanspeech(i,:)));
end
...
start=l+1;
for i=2:totseg−1
cleanSpeech(start:start+(l−overlap))=...
cleanspeech(i,overlap:end);
start=start+l−overlap−1;
end
cleanSpeech(start:length(y))=cleanspeech(totseg,...
1:(length(y)−start)+1);
...
subplot(3,1,1);plot((1:length(z))/fs,z);title('original speech');
axis([0, length(z)/fs, −1, 1]);
subplot(3,1,2);plot((1:length(y))/fs,y,'k');
title('corrupted speech');axis([0, length(y)/fs, −1, 1]);
subplot(3,1,3);plot((1:length(cleanSpeech))/fs,cleanSpeech,'r');
title('cleaned speech');axis([0, length(cleanSpeech)/fs, −1, 1]);
wavplay(y,fs);
wavplay(cleanSpeech,fs);
wavwrite(cleanSpeech,fs,strcat(saveToPath,filename,' Q',...
num2str(m),' ',noiseType,' sn',num2str(SNR(snri)),...
' enhanced.wav'));
saveas(figure(2),[saveToPath,'Waveform ',filename,' Q',...
num2str(m),' ',noiseType,' sn',num2str(SNR(snri))]);
...
saveas(figure(3), [saveToPath,'Spectrogram ',filename,' Q',...
num2str(m),' ',noiseType,' sn',num2str(SNR(snri))]);

%quantitative measure of noise removed − Seg SNR
disp('The segmental snr before processing is :');
segsnr before = 20*segsnr before/totseg
disp('The segmental snr after processing is :');
segsnr after = 20*segsnr after/totseg

%PESQ
psq = pesq(fs,strcat(soundpath,'clean\',filename,'.wav'),...
strcat(saveToPath, filename,' Q',num2str(m),' ',noiseType,...
' sn',num2str(SNR(snri)),' enhanced.wav'));
fprintf(fileID,'%f %s %s %d %f %f %f %d\r\n', R,...
['Q',num2str(m),'=',num2str(mean(Q arr))],...
['n=',num2str(log10(mean(Q arr)))], SNR(snri),...
segsnr before, segsnr after, psq, round(mean(order)));
close all;
end
end
fclose('all');
